...
module.exports = {
  ...
  bulkUpload: async (req, res) => {
    try {
      ...
      for (let row of rows) {
        ...
        let finalEmail =
          email && email !== "n/a" ? email : `user_${mobile}@example.com`;

        let user = await User.findOne({
          $or: [{ email: finalEmail }, { mobile }],
        });
  ...
  addNewUserToFirm: async (req, res) => {
    try {
      ...
      if (!firmId || !name || !email) {
        return res
          .status(400)
          .json({ message: "firmId, name & email are required" });
      }
      ...
      if (!newUser) {
        return res
          .status(500)
          .json({ message: "Unexpected error — user not created" });
      }
      ...
      return res.json({
        message: `User ${
          isExistingUser ? "added to firm" : "created and added to firm"
        } successfully`,
        data: {
          userId: newUser._id,
          name: newUser.name,
    ...
    try {
      ...
      if (!firm) {
        ...
        if (Array.isArray(categories) && categories.length > 0) {
          const newCategories = [
            ...new Set([...firm.categories, ...categories]),
          ];
  ...
  addUserWithFirm: async (req, res) => {
    try {
      ...
      if (!name || !email || !firmName) {
        return res
          .status(400)
          .json({ message: "Name, email, and firm name are required" });
      }
  ...
  searchFirms: async (req, res) => {
    try {
      ...
      const firms = await Firm.find({
        $or: [{ name: regex }, { gst: regex }, { address: regex }],
      })
